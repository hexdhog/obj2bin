...
assert tuple(map(int, platform.python_version_tuple()))[0:2] >= (3, 7), "unsupported python version (>=3.7)"
...
class Const(FrameField):
  def __init__(self, value: object, fmt: str, order: str = ">", enc: Callable[..., Any] | Iterable[Callable[..., Any]] | None = None, dec: Callable[..., Any] | Iterable[Callable[..., Any]] | None = None):
    self.value, self.fmt, self.enc, self.dec = value, order + fmt, enc, dec

class Field(FrameField):
  def __init__(self, fmt: str, stop: Callable[..., Any] | Iterable[Callable[..., Any]] | None = None, order: str = ">",
               enc: Callable[..., Any] | Iterable[Callable[..., Any]] | None = None, dec: Callable[..., Any] | Iterable[Callable[..., Any]] | None = None, meta: bool = False):
    self.fmt, self.stop, self.enc, self.dec, self.meta = order + fmt, stop, enc, dec, meta
...
def frame(**attrs: Const | Field | Child) -> Callable[[type], type]: return lambda x: setattr(x, _PATTRS, attrs) or dataclass(x) # type: ignore[func-returns-value]
def calcsize(obj: object) -> int:
  ...
  for name, attr in getattr(obj, _PATTRS).items():
    if isinstance(attr, Child):
      ...
      size += st.calcsize(attr.fmt.format(**vals)) * mult
  return size
...
def encode(obj: object, buffer: bytearray | None = None, offset: int = 0) -> tuple[bytes, int]:
  ...
  for name, attr in getattr(obj, _PATTRS).items():
    if isinstance(attr, Child):
      ...
      vals[name], size, stop = val, st.calcsize(fmt), getattr(attr, _STOP, None)
      if stop is None:
        ...
        st.pack_into(fmt, buffer, offset, *val)
        offset += size
      else:
        for v in [*totuple(val), attr.encode(stop)]:
          st.pack_into(fmt, buffer, offset, v)
          offset += size
...
def decode(ptype: _T, buffer: bytes | bytearray, offset: int = 0) -> tuple[_T, int]: # noqa: C901
  ...
  for name, attr in getattr(ptype, _PATTRS).items():
    if isinstance(attr, Child):
      ...
      if stop is None:
        vals[name], offset = attr.decode(st.unpack_from(fmt, buffer, offset)), offset + st.calcsize(fmt)
      else:
        vals[name] = []
        while True:
          v, offset = attr.decode(st.unpack_from(fmt, buffer, offset)), offset + st.calcsize(fmt)
          if v == stop: break
          vals[name].append(v)
